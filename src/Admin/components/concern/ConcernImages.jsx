...
import
{
    ...
    clearError,
    clearCurrentConcern
} from '../../../store/slices/concern.slice';

const ConcernImages=() =>
{
    const dispatch=useDispatch();
    const {concernId}=useParams();
    const {currentConcern, loading, imageLoading, error}=useSelector(state => state.concern);
    ...
    const handleUpload=async () =>
    {
        ...
        const result=await dispatch(uploadImage({concernId, formData}));
    ...
    const handleDeleteImage=async () =>
    {
        ...
        {
            await dispatch(removeImage(concernId));
            dispatch(fetchConcernById(concernId));
        }
    };
    ...
    return (
        <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-[#1a472a] hover:text-[#fffcef]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                </Link>
                <div>
                    <h2 className="text-2xl font-bold text-[#1a472a]">
                        Manage Image: {currentConcern?.name||'Concern'}
                    </h2>
                    <p className="text-[#1a472a] text-sm">Upload a single image for this health concern</p>
                </div>
            </div>
            ...
            <div className="bg-white shadow p-6 mb-6">
                ...
                {currentConcern?.imageUrl? (
                    <div className="flex flex-col items-center">
                        <div className="relative group">
                            <img
                                src={currentConcern.imageUrl}
                                alt={currentConcern.name}
            ...
            <div className="bg-white shadow p-6 mb-6">
                <h3 className="text-lg font-semibold text-[#1a472a] mb-4">
                    {currentConcern?.imageUrl? 'Replace Image':'Upload Image'}
                </h3>
                ...
                <div
                    ...
                    className={`border-2 border-dashed p-8 text-center cursor-pointer transition ${dragActive
                        ? 'border-[#1a472a] bg-[#d4edda] hover:border-[#145214] hover:bg-[#1a472a]/20'
                        :'border-[#1a472a] hover:border-[#145214] hover:bg-[#1a472a]/10'
                        }`}
                >
                    <input
                        type="file"
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <button
                                    ...
                                    disabled={imageLoading}
                                    className="mt-2 px-6 py-2 bg-[#1a472a] text-[#fffcef] rounded-lg hover:bg-[#145214] disabled:bg-[#1a472a]/50 transition flex items-center gap-2"
                                >
                                    {imageLoading? (
                                        <>
            ...
            <div className="mt-6 flex gap-3">
                <Link
                    to={`/admin/concerns/edit/${concernId}`}
                ...
                <Link
                    to="/admin/concerns"
                    className="px-4 py-2 bg-[#fffcef] border border-[#1a472a] text-[#1a472a] rounded-lg hover:bg-[#e6e4d9] transition"
                >
                    Back to Concerns
                </Link>
            </div>
