...
import {getProductsByConcernSlug} from '../../api/product.api'
...
const ConcernProducts=() =>
{
    ...
    const fetchProducts=async (page=1) =>
    {
        ...
        try
        {
            const response=await getProductsByConcernSlug(slug, page, 16)
    ...
    if (loading)
    {
        return (
            <div className="min-h-screen flex justify-center items-center bg-[#fffcef]">
                <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-[#004411]"></div>
            </div>
        )
    }
    ...
    if (error)
    {
        return (
            <div className="min-h-screen flex flex-col justify-center items-center bg-[#fffcef] gap-4">
                ...
                <button
                    onClick={() => navigate('/')}
                    className="bg-[#004411] text-[#fffcef] px-6 py-2 rounded-lg hover:bg-[#003300] transition-colors"
                >
                    Go Back Home
                </button>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#fffcef]">
            {/* Hero Section with Concern Info */}
            <div className="bg-[#fffcef] text-[#1a472a] py-12 px-4 md:px-8 lg:px-16">
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        {/* Concern Image */}
                        {concern?.imageUrl&&(
                            <div className="w-32 border-dashed  h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-[#1a472a] shadow-xl flex-shrink-0">
                                <img
                ...
                <div className="max-w-7xl mx-auto  ">
                    {products.length>0? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 md:gap-8">
                                {products.map((product) =>
                                {
                                    ...
                                    return (
                                        <div
                                            key={product.id}
                                            className="bg-[#fffcef]  p-2 shadow-md overflow-hidden cursor-pointer group hover:shadow-xl transition-all duration-300 border border-gray-100"
                                            onClick={() => handleProductClick(product)}
                                        >
                                            {/* Image */}
                                            <div className='h-full w-full border-2 border-dashed border-[#1a472a] '>

                                                <div className="relative h-48 md:h-56 lg:h-64 bg-[#fffcef] overflow-hidden">
                                                    <img
                                ...
                                <div className="flex justify-center items-center gap-2 mt-12">
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 ${pagination.hasPrevPage
                                            ? 'bg-[#004411] text-[#fffcef] hover:bg-[#003300]'
                                    ...
                                    <div className="flex items-center gap-1">
                                        {Array.from({length: pagination.totalPages}, (_, i) => i+1).map((page) =>
                                        {
                                            ...
                                            if (!showPage)
                                            {
                                                ...
                                                {
                                                    return (
                                                        <span key={page} className="px-2 text-[#66a576]">
                                                            ...
                                                        </span>
                                                    )
                                                }
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-10 h-10 rounded-lg font-medium transition-all duration-300 ${page===pagination.currentPage
                                                        ? 'bg-[#004411] text-[#fffcef]'
                                                        :'bg-gray-100 text-gray-700 hover:bg-[#66a576]'
                                                        }`}
                                                >
                                                    {page}
                                                </button>
                        ...
                        <div className="text-center py-16">
                            ...
                            <button
                                onClick={() => navigate('/')}
                                className="bg-[#fffcef] text-[#004411] px-6 py-3 rounded-lg hover:bg-[#e6e2d3] transition-colors"
                            >
                                Browse All Products
                            </button>
                        </div>
